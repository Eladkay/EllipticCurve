...
class Vec2d(val x: Double, val y: Double) {
    ...
    companion object {
        ...
        fun of(string: String): Vec2d? {
            if (!isValid(string.trim())) return null
            val groups = MATCHING_REGEX.matchEntire(string.trim())!!.groups
    ...
    fun truncate(num: Int): Vec2d {
        val pow10 = 10.0.pow(num.toDouble())
        return map { (it * pow10).toInt() / pow10 }
    }

    fun round(num: Int): Vec2d {
        val pow10 = 10.0.pow(num.toDouble())
        return map { round(it * pow10) / pow10 }
    }
